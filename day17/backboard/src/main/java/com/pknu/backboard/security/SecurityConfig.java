...
public class SecurityConfig {
        ...
        SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
                ...
                ; // ; 을 분리해 놓으면 chain method 추가시 간편함 (지금은 Prettier 때문에 불가)

                return http.build();
        }

        // 스프링 시큐리티로 MemberSecurityService와 패스워드 등을 내부적으로 사용, 인증 + 권한 부여 프로세스 처리
        @Bean
        ...
        // 회원가입, 로그인 시 동일하게 사용(로그인 시 스프링 시큐리티가 자동으로 사용)
        @Bean
        PasswordEncoder passwordEncoder() {
                return new BCryptPasswordEncoder(); // 회원가입 패스워드 암호화 시 사용한 인코더와 동일
        }
}
